...
**🏆 Создан для NOT Back Contest**: Соревнование backend-разработчиков, требующее от участников создать полный сервис flash-распродажс нуля, используя Go, Redis, Postgres и Docker — без фреймворков и с минимальными зависимостями.

## Простая Архитектура 🎯

Этот сервис спроектирован с упором на **простоту и производительность**:

- **Единственная PostgreSQL база** - Одна база данных обрабатывает всё постоянное хранение (есть возможность маштабирования)
...
- **Высокая конкурентность** - Единственный экземпляр может эффективно обрабатывать множество одновременных клиентов
...
- ✅ **Высокая конкурентность** - Потокобезопасные операции с производительностью 17M+ оп/сек
